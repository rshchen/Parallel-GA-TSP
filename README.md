...
*註：*
1. **Gap % (相對誤差)**
   衡量演算法的「尋優精度」，即實測結果與理論極限的距離。
   $$\text{Gap (\%)} = \frac{\text{Best} - \text{BKS}}{\text{BKS}} \times 100\%$$
   * **BKS (Best Known Solution)**：指 TSPLIB 官方公佈的已知理論最優解（基準常數）。
   * **Best**：指本演算法在 10 次獨立隨機實驗中搜尋到的最短路徑長度。
   * **意義**：數值越低代表演算法越接近全域最佳解。

2. **CV % (變異係數)**
   衡量演算法的「穩健性 (Robustness)」，反映隨機性對結果的影響程度。
   $$\text{CV (\%)} = \frac{\sigma}{\mu} \times 100\%$$
   * **$\sigma$ (Standard Deviation)**：10 次實驗結果的標準差。
   * **$\mu$ (Mean)**：10 次實驗結果的平均值。
   * **意義**：反映演算法在不同隨機種子下輸出結果的離散程度。數值越低，代表演算法表現越穩定，具備高可重現性。

3. **平均耗時 (Average Execution Time)**
   衡量系統的「運算效率」。
   $$\text{Avg. Time} = \frac{1}{N} \sum_{i=1}^{N} T_i$$
   * **意義**：在 Release 編譯環境下，10 次測試的平均執行時間。
   * **註記**：本指標配合 **7.09x 並行加速比**，證明了本系統在處理大規模組合優化問題（如 $n=150$）時，具備商用級的響應時效。
